...
MultiChat 是一个开源的、支持视觉（图片）的对话语言模型，具有约70+亿参数，我们固定视觉模型、语音模型、对话模型，通过设计类语言化桥模块将视觉和语音进行类语言化后输入给对话模型，参考了Blip2并基于ViT-g和ChatGLM-6B模型。目前MultiChat支持视觉和文本的对话，我们后续会开源支持视觉、文本、语音的多模态对话模型。
